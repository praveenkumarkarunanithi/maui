...
namespace Maui.Controls.Sample
{
    ...
    internal static class ThemeElement
    {
        ...
        internal static void InitializeThemeResources(Element element, string controlKey)
        {
            ...
            element.SetDynamicResource(ThemeElement.CommonThemeProperty, "CustomTheme");
        ...
        /// <summary>
        /// WORKAROUND: Refreshes visual states for all controls with visual state groups after theme change.
        /// This ensures DynamicResource setters in visual states get updated with new theme values.
        /// </summary>
        /// <param name="rootElement">Root element to search for controls</param>
        public static void RefreshVisualStatesAfterThemeChange(Element rootElement)
        {
            if (rootElement == null) return;

            // Refresh visual states for the current element if it has them
            if (rootElement is VisualElement visualElement && visualElement.HasVisualStateGroups())
            {
                ...
                foreach (var group in groups)
                {
                    ...
                    {
                        // Force re-application of current state to refresh DynamicResource setters
                        VisualStateManager.GoToState(visualElement, group.CurrentState.Name);
                    }
                }
            }

            // Recursively process all child elements using IVisualTreeElement
            if (rootElement is IVisualTreeElement visualTreeElement)
            {
                foreach (var child in visualTreeElement.GetVisualChildren())
                {
                    if (child is Element childElement)
                    {
                        RefreshVisualStatesAfterThemeChange(childElement);
                    }
                }
            }
        }
